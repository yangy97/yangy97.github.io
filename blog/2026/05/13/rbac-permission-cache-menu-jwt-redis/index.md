...
---
每次请求扫库拼权限在 **QPS**（每秒查询数，衡量接口吞吐）稍高时就会成为瓶颈。本篇讨论三种缓存层级：**HTTP 内会话**、**Redis**（内存键值存储，常用于缓存与会话）侧权限集、**JWT claims**（JWT 载荷中的身份/版本字段），以及前端 **`routes → menuTree`** 合并时要注意的 **父子依赖**。

***

### 一、会话快照：`userId → permSetVersion`

**`permSetVersion`（权限集版本号）**：角色绑定变更时递增，用于让 Redis 缓存与 JWT 同时失效而不扫全表。
...
**JWT**（JSON Web Token，无状态身份凭证）宜放 **身份 + 版本指针**，不宜塞完整权限列表。
