...
---
`app/extend/` 是 Egg.js 里最容易「顺手往里塞全局」的地方：**ctx.xxx**（Context，单次请求的上下文对象）、**app.xxx**（Application，进程级应用实例）、**helper.xxx**（模板辅助方法） 一旦泛滥，类型推导与单测都会痛。本篇聚焦 **Loader 何时合并 extend**、**三类挂载的差异**、以及 **和插件 extend 的优先级直觉**（细节以当前 Egg 版本文档为准）。
...
| **helper** | 视图 / 模板侧格式化（若仍 SSR（Server-Side Rendering，服务端渲染）片段）；也可给 Controller 当「纯函数外壳」 | 在 helper 里直连 DB（绕过 Service 分层） |
...
插件同样可以声明 extend。合并策略可理解为：**框架默认 → 插件 → 应用**，后者覆盖同名需谨慎（Loader 是 Egg 启动时按约定加载并合并各类文件的机制）。团队规范建议：
...
* **middleware（中间件，按顺序拦截请求的可插拔函数链）**：横切、顺序敏感、可能短路响应。
