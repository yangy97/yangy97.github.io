...
---
当同一套能力要在 **多个 Egg.js（基于 Koa 的企业级 Node.js 框架）应用**（BFF、开放平台、内部工具）复用时，自定义 **`egg-xx`** 插件优于复制 `app/extend`。本篇梳理：**configWillLoad → didLoad → willReady → didReady → serverDidReady → beforeClose** 各阶段适合做什么，以及 **`app.addSingleton`** 一类模式的取舍。
...
* 暴露 **清晰的 config schema（配置结构约定，可用 TS / Joi 文档化）**
...
* **willReady / didReady**：连接池预热、订阅外部 broker（消息中间件）。
...
数据库、Redis（内存键值存储，常用于缓存与会话）、MQ 生产者常用 **单例**。**注意**：测试环境 **parallel** 跑单测时要支持 **mock / 替换**，否则全局污染。
