...
---
`egg-security` 集成了 helmet 一类 HTTP 头、CSRF（Cross-Site Request Forgery，跨站请求伪造）、safeRedirect 等能力。**管理后台 + 同源 SPA（Single Page Application，单页应用）** 与 **开放 API + 移动客户端** 对 CSRF 的需求截然不同；本篇按场景拆解 **什么时候该关、关了用什么补偿**，避免「复制禁用 csrf 的祖传配置」。

***

### 一、CSRF 在 Egg 里防护的是什么

**CSRF** 典型场景：**浏览器携带会话 Cookie** 时，恶意站点诱导用户发起 **状态变更请求**。Egg 通过 **token / referer 白名单**（具体策略以版本文档为准）校验「请求是否来自可信前端」。
...
* **生产前后端分域**：Cookie 要 **`SameSite=None; Secure`** 才可能跨站携带，同时 CSRF 策略要与 **网关 CORS（Cross-Origin Resource Sharing，跨域资源共享）**、**凭证模式**一致——这块是事故高发区，建议画一张 **Cookie + CORS + CSRF** 矩阵评审。
...
* **`X-Frame-Options` / CSP frame-ancestors**（CSP，Content Security Policy，内容安全策略；管理后台是否允许嵌 iframe）
