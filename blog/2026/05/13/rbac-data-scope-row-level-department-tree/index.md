...
---
接口 **RBAC**（Role-Based Access Control，基于角色的访问控制）鉴权通过后，用户仍可能 **看见不该看的行**。**数据权限（Data Scope，行级可见范围）** 常见模型：**全部 / 本部门及子部门 / 仅本人 / 自定义列表**。本篇写 **约束下发通道**、**SQL 拼接陷阱**、以及 **审计如何记录 scope**。
...
**部门树** 决定 `DEPT_SUBTREE` 过滤范围；**递归 CTE**（Common Table Expression，SQL 递归查询子孙节点）适合 MySQL 8+ 在线过滤。
