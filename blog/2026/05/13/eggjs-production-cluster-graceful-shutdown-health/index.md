...
---
从「本地 `egg-bin dev`」到「K8s（Kubernetes，容器编排平台）滚动发布」，差距在 **进程信号**、**存量连接排空**、**探测接口**。本篇对齐：**SIGTERM 处理**、**liveness vs readiness**、以及与 **上游网关超时** 的配合。

***

### 一、优雅停机（graceful shutdown，有序关闭服务、排空在途请求）

收到 **SIGTERM**（终止信号，容器编排滚动发布时常见）：

1. **停止接受新连接**（框架层关闭 server）。
2. **等待在途请求完成**，设置 **上限时间**（如 30s）；超时强制退出。
3. **关闭 DB pool / Redis / MQ**，防止进程挂了连接泄漏。

Egg / **cluster**（多 Worker 进程集群模式）下要确保 **每个 Worker** 都执行 cleanup；**Agent**（代理进程，集群单例）也要 **beforeClose**。
...
| 探针 | 问的问题 | 典型实现 |
|-----|-----------|---------|
| **liveness**（存活探针，判断进程是否死锁） | 进程是否死锁 | 轻 ping，不做下游 |
| **readiness**（就绪探针，判断能否接流量） | 能否接流量 | 可选检查 Redis/MySQL **连通非深度查询** |
