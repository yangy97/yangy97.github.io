...
---
`egg-sequelize` 把 Model（ORM 数据模型）绑定进 **`ctx.model`**。本篇不写 Hello World，而写 **线上真金白银四类问题**：迁移不回滚、长事务锁表、`UPDATE … WHERE version` 乐观锁、以及列表页的 **N+1（一次查询主表后再逐条查关联，导致 1+N 次 SQL）**。

***

### 一、迁移：向前兼容与回填

* **加列默认值**：大表在线 DDL（Data Definition Language，表结构变更）要考虑锁；优先 **nullable → 回填 → NOT NULL** 两阶段。
...
Controller **不写事务**；Service 方法 **`ctx.model.transaction(fn)`** 包住一组写入。**隔离级别**：默认 Repeatable Read（MySQL InnoDB 默认可重复读）下注意 **幻读（同一事务内重复查询结果集变化）敏感的业务**（如配额扣减）是否要 Serializable 或用 **SELECT … FOR UPDATE（行级排他锁）** 收紧临界区。

**反模式**：在 middleware 里开事务——请求没到 Controller 就可能超时提交失败。

**乐观锁（Optimistic Locking，用 version 字段检测并发写冲突）** 见下一节；本节先强调事务应 **短、边界清晰**。
