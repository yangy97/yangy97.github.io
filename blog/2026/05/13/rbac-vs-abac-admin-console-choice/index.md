...
---
**RBAC**（基于角色的访问控制）回答「你是谁的角色」；**ABAC**（Attribute-Based Access Control，基于属性的访问控制）回答「在当前上下文是否允许」。后台系统常见诉求：**按部门、数据标签、金额阈值** 动态放行——这已经触碰 ABAC。**数据权限 / Data Scope**（行级可见范围）常与 RBAC 组合。本篇给 **选型阈值** 与 **渐进演进**，避免一上来 Cel / OPA 全家桶。
...
**属性** 指运行时上下文（部门 id、区域、金额、客户集合等），由策略引擎或规则 DSL 求值。
