...
---
Egg.js 应用常见的内存问题往往不是「GC（Garbage Collection，垃圾回收）坏了」，而是 **闭包持有大对象**、**全局缓存无界**、**Promise 堆积**、或 **序列化超大 JSON**。本篇给一条 **可操作的排障路径**，工具以 **Node 内置 inspector / clinic / heapdump** 为例（版本差异自行核对文档）。
...
* **RSS（Resident Set Size，进程实际占用的物理内存）高但 heap 正常**：Buffer、native 模块、或 **碎片**。
* **外部内存**：例如大量 **Buffer / TypedArray**。

监控里同时看 **`uv_threadpool` 队列**（libuv 线程池，Node 用于异步 FS/crypto 等；虽非内存指标）辅助判断是否 **同步 FS/crypto** 拖尾。
...
* **峰值后 plateau**：疑似泄漏。
* **发布新版本后**：对比两份 heap snapshot（堆内存快照） **Delta**，找 Retainers（仍引用目标对象、阻止 GC 的持有者链）。
