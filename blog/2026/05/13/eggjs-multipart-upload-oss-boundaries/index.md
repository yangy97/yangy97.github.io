...
---
后台「导入 Excel」「贴图富文本」「工单附件」都会触发上传。Egg 侧既有 **`multipart`（HTTP 多部分表单上传解析）** 走磁盘缓存的模式，也有 **前端直传 OSS（Object Storage Service，对象存储）** 只走后端签名的模式。本篇对比 **瓶颈、鉴权与恶意文件** 三道防线。
...
* **并发**：上传风暴要打 **全局限流**（令牌桶，按固定速率发放许可的流量控制）或网关层 QoS（服务质量保障）。

***

### 二、OSS 直传：STS / Post Policy

流程：**Egg 校验登录与权限 → 签发短时凭证或 Policy（Post Policy，限制上传条件的签名策略） → 浏览器 PUT 到 OSS → 回调或前端带回 object key → Egg 落库元数据**。

关键点：

* **回调验签** 不可省略；否则伪造成功上传。
* **MIME（Multipurpose Internet Mail Extensions，文件类型标识）与后缀双校验**：OSS 可配置 **Content-Type** 限制。
