...
---
权限系统上线后，真正的风险来自 **人**：给自己加角色、导出全库、批量改价。**审计（Audit Log）** 不只是「记日志」，而要支持 **检索、告警、法务取证**；**越权 / 特权提升（Privilege Escalation）** 指在未授权前提下获得更高权限或更广数据可见性。本篇给 **最小日志字段集** 与 **两类告警**。
...
对接 **SIEM**（Security Information and Event Management，安全信息与事件管理） 时用 **固定 schema** 便于关联。
