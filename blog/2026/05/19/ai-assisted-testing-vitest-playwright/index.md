...
---
AI 写测试 **快但不准**——边界条件、异步时序、mock 范围常错。正确姿势是：**人定验收清单，AI 出草稿，CI 裁决**。本篇给 **单测 / 集成 / E2E** 三类提示模板与审查要点。
...
| 层 | 工具 | AI 适合生成 |
|----|------|-------------|
| 单元 | Vitest | 纯函数、util、store action |
| 集成 | Vitest + MSW | API 层、composable |
| E2E | Playwright | 关键路径 smoke（需人工审 selector） |
...
* \[ ] 是否 **假阳性**（没 assert 行为）
* \[ ] mock 是否 **过宽**（mock 了整个 axios）
* \[ ] 异步是否 `await` / `flushPromises`
...
使用 @vue/test-utils，不要 snapshot 整页
```
...
**必须人审**：selector 是否稳定、是否测了 **关键断言** 而非只 `toHaveURL`。
