...
---
Pinia（Vue 3 官方状态库）是 Vue 3 官方推荐的状态库：**无 mutations**（直接改 state）、**TS**（TypeScript）友好、可按 store 拆模块\*\*。本篇讲 **怎么拆 store、何时持久化、如何与路由守卫/权限** 配合，避免「一个大 store 装天下」。
...
\==原则==：**跨页共享且非服务端真相**（单一数据源在 API） 的放 Pinia；服务端真相 **仍以 API 为准**，store 是缓存。

***

### 二、Setup Store（组合式写法 store）示例
