...
---
框架题（Vue/React 原理与对比）几乎 **每面必问**。本篇按 **Vue2/3 对比、响应式、生命周期、状态、React（UI 组件库） hooks** 组织答法，并链到本站 Vue 专题。
...
| 点 | Vue2 | Vue3 |
|----|------|------|
| API | Options API 为主 | \*\*\*\***Composition API**（组合式，逻辑聚合）**（组合式，逻辑聚合）**（组合式，逻辑聚合） |
| 响应式 | `Object.defineProperty`（属性级拦截） | `Proxy`（对象级拦截） |
| 根节点 | 单根 | 多根 Fragment |
