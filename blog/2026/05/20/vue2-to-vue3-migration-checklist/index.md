...
---
Vue 2 **EOL**（End of Life，停止维护） 后，存量项目迁移是 **渐进过程**，不是一夜重写。本篇给 **分阶段 checklist** 与 **高频 Breaking 对照表**，适合 Options API 老项目。
...
| 3 | `@vue/compat`（迁移兼容包） 跑 Vue 3 兼容模式 | 中 |
...
| Vue 2 | Vue 3 |
|-------|-------|
| `new Vue()`（Vue 2 根实例） | `createApp()`（Vue 3 应用工厂） |
