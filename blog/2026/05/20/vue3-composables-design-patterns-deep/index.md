...
---
`useXxx` 是 Vue 3 **组合式 API**（Composition API，逻辑按功能组织） 的核心复用单元。写得好的 composable **像小型服务**；写得坏的会变成 **隐式全局状态泥球**（多处共享可变状态、边界不清）（多处共享可变状态、边界不清）（多处共享可变状态、边界不清）。本篇在[《组合式 API 常用写法》](/2022/04/20/vue3-composition-api-patterns/)基础上加深 **命名、参数、副作用、测试**。
...
| 有明确输入输出（纯函数式接口）（纯函数式接口）（纯函数式接口） | 强绑定某页面 DOM 结构 |
