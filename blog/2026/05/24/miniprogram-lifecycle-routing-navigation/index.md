...
---
小程序和 H5 最大的结构差异之一是：**页面由客户端托管的「栈」**（page stack，后进先出） 管理，生命周期分 **App 级** 与 **Page 级**，参数传递还有 **冷启动**（进程新建）/ **热启动**（进程仍在后台） 之分。搞不清这三块，很容易出现 **参数丢失、返回刷新异常、分享进错页**。

本篇以微信小程序为主（其他平台概念相近），把 **生命周期 → 路由 API → 参数与场景值 → 常见坑** 串成一条线。

***

### 一、App 级生命周期（小程序全局 `App()` 钩子）（小程序全局 `App()` 钩子）（小程序全局 `App()` 钩子）
