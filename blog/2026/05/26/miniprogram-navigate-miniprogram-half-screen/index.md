...
---
业务常要 **从 A 小程序打开 B**（支付、联名活动、服务商能力），或使用 **半屏小程序**（`openEmbeddedMiniProgram`，嵌入式打开）（`openEmbeddedMiniProgram`，嵌入式打开）（`openEmbeddedMiniProgram`，嵌入式打开） 降低跳出感。本篇讲 **跳转配置、参数传递、返回栈与审核**。
...
| API | 场景 |
|-----|------|
| `wx.navigateToMiniProgram`（跳转独立小程序）（跳转独立小程序）（跳转独立小程序） | 打开其他小程序 |
