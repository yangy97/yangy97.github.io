...
---
中后台常要 **预览 PDF/Office、带水印、按权限下载**。前端不能「隐藏按钮就安全」，必须 **服务端鉴权 + 短期 URL**（**签名 URL**，带过期时间的对象存储直链）。本篇串完整链路，并补 **ticket（一次性预览凭证）**、OnlyOffice/pdf.js 集成与 **审计** 细节。
...
与 **RBAC**（基于角色的访问控制）关系：列表层 **数据权限** 过滤 fileId（见 [行级数据权限](/2023/07/12/rbac-data-scope-row-level-department-tree/)）。

***

### 二、Ticket 与签名 URL

#### 2.1 签发（**BFF**（Backend For Frontend，为前端聚合 HTTP 的 Node 网关层） / Egg）
