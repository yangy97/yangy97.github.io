...
---
某 **机构客户门户** 采用 **Egg 单应用 + Vue 3 SPA**（单页应用，浏览器内客户端路由）：多套地域部署共用一套仓库，通过 **环境变量** 切换站点；权限上既要支撑 **个人 / 机构 / 合规 / 内部代管** 等产品形态，又要让 **同一套前端路由与后端接口** 在不同形态下自动 **收窄菜单与 API 可见范围**。工程上使用 **`sceneManager`**（策略链管理器，按序遍历各 **`IScene`**（场景策略接口：各实现提供 `isMatch` 判定当前请求是否命中、`getConfig` 产出该形态的功能/权限配置快照））在请求级产出场景配置，再与 **中心化机构策略**、**使用者 RPC 权限** 合成 **`flatCaps`**（扁平能力键列表，原子权限码的逗号/分隔串，供首屏与路由裁剪） 与 **ACL 树**（嵌套权限目录）。按 **仓库拓扑 → 端到端流程 → 示意代码 → 场景理论** 展开。**文中所有代码块均为架构示意**，字段名、路径、类名已与真实仓库 **刻意不一致**，不可复制粘贴进业务工程。
...
**分层口诀**：**场景决定「能开通哪些功能模块」**；**机构关停配置决定「该机构实际开通了哪些」**；**使用者绑定决定「这个人能用哪些原子权限 key」**。前端 **`routesBuilder`**（名称泛指「路由装配器」）只做 **UX 层裁剪**，不能替代服务端 **`assertGate(...)`（接口门闸：校验当前会话是否持有某能力键，无权则 403）** 一类接口鉴权。

***

### 端到端流程：首屏 HTML → Vue 路由 → 权限树接口

整体上存在 **两条「权限形状」出口**：其一是 **首屏嵌入的 `flatCaps`**（扁平能力键，真实项目常用逗号串注入 HTML）；其二是 **「权限目录」JSON 接口**（下文用虚构路径 **`GET /api/v1/access/catalog`** 表示**这一类**接口）。二者底层都走 **同一套「场景快照 × 机构策略 × 用户绑定」合成**，因此 **场景顺序或地域弄错** 会 **同时弄歪菜单与配置页**。
...
每个 **`IScene` 实现** 统一契约：**异步判断是否命中（`isMatch`）**、**异步产出配置（`getConfig`）**。
...
**`IScene`**：每个地域/产品形态一个实现；**`flatCaps`** 由合成管线产出后注入首屏，供路由 **`meta.gates`** 裁剪；业务 API 仍须 **`assertGate`** 二次校验。
