...
---
## 测试不是越多越好，而是 **在对的层花对的成本**。本篇给 **测试金字塔**（Test Pyramid，少量慢测在上、大量快测在下的分层思想，前端版）、各层职责、与现有 Playwright/Vitest 文的衔接。
...
| 层 | 测什么 | 工具 | 速度 |
|----|--------|------|------|
| 单测 | 纯函数、Pinia action、**composable**（Vue 组合式逻辑复用单元） | **Vitest**（与 Vite 同生态的测试运行器） | 毫秒 |
| 集成 | 组件 + mock API | Vitest + **MSW**（Mock Service Worker，拦截 HTTP 返回假数据） | 秒级 |
| **E2E**（End-to-End，真浏览器端到端） | 真浏览器、真环境 | **Playwright** | 分钟 |
...
* **性能预算 CI**（Performance Budget，构建产物体积或 Web Vitals 超阈则 fail）与 **smoke E2E**（冒烟级最小主路径用例）并行，不互相替代
