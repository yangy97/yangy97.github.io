...
---
「跨域失败」在 Network 里表现为 **CORS**（跨域资源共享） 红字，但根因可能是 **Cookie**（浏览器存储凭证） 没带、**预检**（OPTIONS 请求） 失败、**SameSite**（跨站 Cookie 策略） 不对。本篇用 **简单模型 + 配置对照** 讲清。

***

### 一、同源 vs 跨域

同源（Same-Origin，协议+域名+端口一致） = **协议 + 域名 + 端口** 全同。\
