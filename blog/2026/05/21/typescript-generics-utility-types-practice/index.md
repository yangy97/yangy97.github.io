...
---
「类型体操」不必为炫技；在中后台里 **最常用的是接口约束、Partial**（全部可选）、**Pick**（选取字段）、**Omit**（排除字段）、**ReturnType**（函数返回类型）。本篇用 **API 封装与表格列配置** 两个场景讲透。

***

### 一、API 响应泛型（`<T>` 占位符约束 data 形状）（`<T>` 占位符约束 data 形状）（`<T>` 占位符约束 data 形状）
