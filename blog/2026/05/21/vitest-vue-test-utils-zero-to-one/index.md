...
---
## Vitest 与 Vite 同生态，**配置复用、HMR 测起来快**（HMR，Hot Module Replacement，改测试文件热更新）。本篇搭好 **Vitest + @vue/test-utils + jsdom**（在 Node 里模拟 DOM 的环境），写第一个组件测试并接进 CI。
...
**@vue/test-utils**（Vue 官方组件测试工具，`mount` 挂载组件实例）：
