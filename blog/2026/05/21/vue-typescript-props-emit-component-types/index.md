...
---
`<script setup lang="ts">` 里 **Props/Emits 类型** 写对了，IDE 才能在整个仓库里传导约束。本篇覆盖 **defineProps**（声明入参类型）**、defineEmits**（声明事件载荷）**、组件库 publish types**（对外导出 .d.ts）。

***

### 一、Props（Vue 3.4+ 泛型 `defineProps<>` 推荐）
...
运行时声明（`type`/`default` 对象） + 类型（需兼容老工具时）：
