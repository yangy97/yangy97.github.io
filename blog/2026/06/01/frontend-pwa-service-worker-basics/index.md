...
---
## 移动端 H5 有 [离线包](/2023/05/18/mobile-h5-offline-package-release-rollback/) 方案；标准 Web 侧对应 **PWA**（Progressive Web App，可安装、可离线的 Web 应用）+ **Service Worker**（SW，浏览器后台脚本，拦截网络请求做缓存）。

### 一、PWA 解决什么问题

| 能力 | 用户感知 | 工程代价 |
|------|----------|----------|
| **可安装** | 桌面/主屏图标，独立窗口 | **manifest.json**（Web App Manifest，安装元数据）+ 图标集 |
| **离线/弱网** | 壳能打开、静态资源可用 | SW 缓存策略 |
| **后台同步** | 表单断网后补传 | **Background Sync**（浏览器后台补传 API，支持有限） |
