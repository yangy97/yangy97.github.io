...
---
Vue 2 源码笔记（[defineProperty / Dep](/2020/12/17/vue-learn-defineProperty/)）讲的是 **Object.defineProperty + Dep/Watcher**；Vue 3 换成 **Proxy**（对象属性拦截代理）+ **effect**（副作用函数，依赖变则重跑）+ **track/trigger**（依赖收集/派发更新），编译器也拆成 **compiler-dom**（DOM 平台编译）/ **runtime-core**（与平台无关的运行时核心）。这篇给已经会用组合式 API 的开发者，补 **原理层因果链**，方便读源码、排性能 bug（如多余 re-render）、写库。
...
与 Vue 2（基于 defineProperty 的响应式）对比：
