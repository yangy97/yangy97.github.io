...
---
## 已有 [Playwright 架构与 Tracing](/2025/11/16/headless-playwright-architecture-tracing-ci/) 讲 Browser/Context/Trace；本篇补 **一条可拷贝的中后台业务链**：**登录态复用 → 列表筛选 → 表单提交 → 断言 Toast/路由**，并带 **Page Object**（页面对象模式，把页面操作与断言封装为类/模块）**与 CI 配置**。

### 一、场景与验收标准

**用户故事**：运营登录后台 → 打开订单列表 → 按状态筛选 → 新建订单 → 看到成功提示 → 列表出现新单。

**E2E**（End-to-End，端到端浏览器测试，smoke 级）断言：
...
e2e/
  ...
  .auth/
    user.json          # globalSetup 登录后 storageState（浏览器 Cookie/LocalStorage 快照，复用登录态）
  pages/
    LoginPage.ts
    OrderListPage.ts
    OrderFormPage.ts
