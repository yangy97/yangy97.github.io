...
---
[Vue 3 响应式与运行时](/2026/06/01/vue3-reactivity-compiler-runtime-deep/) 讲了 Proxy、patchFlag 与 patch 流程；本篇专讲 **@vue/compiler-core** 流水线：**parse → transform → generate**，与 runtime 那篇 **互补**。读完后应能解释「模板里 `v-if` 为什么变成 `block` + `patchFlag`」。
...
* **compiler-dom**：HTML 特有（如 `v-html`、`style` 解析）。
* **compiler-core**：与平台无关的 AST、指令、表达式。
