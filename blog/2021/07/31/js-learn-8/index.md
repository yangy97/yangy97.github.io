...
#### 如果要确定某个属性是否存在于原型（prototype，实例共享属性链）上
...
`for-in 循环和 Object.keys()`（仅枚举自有可枚举键） 的枚举顺序是不确定的`
`Object.getOwnPropertyNames()`、`Object.getOwnPropertySymbols()`和 **Object.assign()**（浅拷贝合并对象） 的枚举顺序是确定性的。先以升序枚举数值键，然后以插入顺序枚举字符串和符号键`
