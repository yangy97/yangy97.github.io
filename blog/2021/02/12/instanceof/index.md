...
---
`******instanceof**（沿 `\[\[Prototype]]`链查构造函数）**（沿`\[\[Prototype]]`链查构造函数）**（沿`\[\[Prototype]]` 链查构造函数）` 判断 **构造函数的 prototype 是否出现在对象的原型链（`__proto__` 链接的对象链）（`__proto__` 链接的对象链）（`__proto__` 链接的对象链）上**。面试常要求手写；生产里更常用 **类型守卫、Array.isArray、duck typing**。本篇补全 **边界、完整实现与替代方案**。
