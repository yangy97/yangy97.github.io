...
---
### BOM（Browser Object Model，浏览器对象模型）（Browser Object Model，浏览器对象模型）（Browser Object Model，浏览器对象模型）

#### setTimeOut中this指向

> 所有超时执行的代码（函数）都会在全局作用域中的一个匿名函数中运行，因此函数中的 this 值在非严格模式下始终指向 window（全局对象）（全局对象），而在严格模式下是 undefined。如果给 setTimeout()提供了一个箭头函数，那么 this 会保留为定义它时所在的词汇作用域
