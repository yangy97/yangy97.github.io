...
---
## Puppeteer、Playwright 底层与 Chromium 对话，主要靠 ==**CDP**（Chrome DevTools Protocol，WebSocket 上 JSON-RPC 的远程调试协议）==：域如 **Page、Network、Runtime、Fetch**。
...
无头浏览器启动时带 **`--remote-debugging-port`**（或 pipe），客户端连上后即可 **订阅事件 + 调方法**。**JSON-RPC**（远程过程调用，请求/响应均为 JSON）承载命令与事件。
