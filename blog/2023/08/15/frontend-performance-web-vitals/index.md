...
---
\==Core Web Vitals==（Google 定义的核心 Web 体验指标集合）是 **用户体验量化** 集合；**实验室（Lighthouse）** 可复现，**RUM**（Real User Monitoring，真实用户监控）反映网络/设备长尾。
...
| 指标 | 测什么 | 常见差因 |
|------|--------|----------|
| ==TTFB==（Time to First Byte，首字节时间，偏服务端/网络） | 从发请求到收到第一个响应字节 | 源站慢、冷启动、未缓存 HTML、TLS 往返 |
| **LCP**（Largest Contentful Paint，视口内最大内容何时绘制完成） | 视口内 **最大** 内容绘制完成 | 大图晚到、阻塞 CSS/JS、字体阻塞、LCP 元素被 lazy |
| **INP**（Interaction to Next Paint，交互到下一次绘制的延迟，多次交互取最差） | 交互到 **下一次绘制** 的延迟 | 长任务、大列表 patch、第三方占线程 |
| **CLS**（Cumulative Layout Shift，累计布局偏移） | 意外布局偏移累计 | 无尺寸媒体、动态插广告、字体 swap 引起换行 |
...
**FID**（First Input Delay，首次输入延迟）只量第一次交互；后台用户 **连续点表格、筛选项**，INP 关注 **最差交互**。优化：
