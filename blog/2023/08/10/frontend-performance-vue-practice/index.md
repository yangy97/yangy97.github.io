...
---
Vue 3 的响应式 + 编译器已经很快，但 ==误用响应式边界==、**巨型列表**、**错误 key**、**首屏瀑布请求** 仍会把 **主线程**（跑 JS、布局、协调绘制的那条线程）和内存打满。
...
| 第三方大实例（地图、图表、编辑器） | ==`shallowRef`==（只追踪 `.value` 替换，不深度代理内部字段）或存实例用 **`markRaw`**（标记对象，跳过响应式代理） |
| 仅防止被代理 | `markRaw` |
...
**`v-memo`**（Vue 3 指令：依赖数组不变则跳过该子树 diff）当父级常更新、但 **行内仅少数字段变** 时：
