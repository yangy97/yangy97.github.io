...
---
## 截图、PDF、打印 CSS、整页滚动截屏，是无头浏览器 ==最高频的工程需求==（报表归档、合规留痕、视觉回归）。另一类痛点是 **flaky**（测试不稳定，本地绿、CI 红）。
...
* **fullPage: true** 会滚动拼接长页，**懒加载图片** 可能只截到占位 → 需先 **滚动触发加载** 或 **mock**（模拟）数据。
...
### 三、**视觉回归**（Visual Regression，对比截图像素/阈值发现 UI 意外变化）：像素 diff 与「语义」回归
