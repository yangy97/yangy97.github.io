...
---
**SSR**（Server-Side Rendering，服务端渲染 HTML）解决 ==首屏 HTML 与 SEO==；**Hydration**（注水：客户端把静态 DOM 接上事件与响应式）解决 **把静态 DOM 变成可交互应用**。代价是：**数据一致性**、**注水主线程成本**、**运维复杂度**。
...
**TTI**（Time to Interactive，页面可稳定交互的时间）与 **INP**（Interaction to Next Paint，交互响应延迟）比单看 LCP 更贴「能点」。用户要的是 **能点**。主包 2MB → parse+hydrate 仍可能 **数秒**。手段：
...
服务端 **边生成边刷 HTML**，可 **更早 TTFB**（Time To First Byte，首字节时间），但 **水合顺序** 与 **脚本依赖** 要设计。Nuxt、Next 等有现成方案；自研成本高。
...
| 场景 | 倾向 |
|------|------|
| 内容/SEO 强依赖 | SSR 或 **SSG**（Static Site Generation，构建期生成静态 HTML） |
| 登录后后台 | SPA + 骨架屏 常更省总成本 |
| 全球静态分发 | SSG + CDN 常优于自建 SSR 集群 |
| 仅部分页面 SEO | 选 **预渲染**（Prerender，构建时为指定路由生成 HTML）子集 |
