...
---
## ==代码分割==（Code Splitting，把 bundle 拆成多块按需加载）决定 **首屏下载多少**、**缓存命中率** 与 **并行请求数**。Webpack 5 里由 **`import()`** 产生 **async chunk**，由 **`splitChunks`** 抽 **vendor / common**，**`runtimeChunk`** 则把 **manifest**（chunk 映射表）从业务 chunk 里拆出，避免 **业务改一行 → vendor hash 全变**。

### 一、三种 chunk 心智模型

| 类型 | 来源 | 典型内容 |
|------|------|----------|
| ==Entry chunk== | `entry`（入口配置） | 入口 + 同步依赖 |
