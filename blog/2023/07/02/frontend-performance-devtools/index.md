...
---
目标：把「慢」拆成 ==可改动的变量==——**哪个资源、哪段 JS、哪次 Layout、是否泄漏**。
...
3. **CPU 6× slowdown** + **Slow 4G**（DevTools 网络/CPU 节流，模拟长尾用户）。
...
* **Metrics**：**LCP**、**TBT**（Total Blocking Time，长任务阻塞总时长）、**CLS**（Cumulative Layout Shift，累计布局偏移，实验口径）。
...
DevTools 的目标是把问题变成 **文件路径 + 函数名 + 资源 URL**；再结合[《网络与静态资源》](/2023/07/21/frontend-performance-network-assets/)《JavaScript 与加载》落地修改。
