...
---
主线程负责：==解析/编译 JS==、执行、样式计算、布局、部分绘制协调。Chrome 把 **>50ms** 的连续任务标为 **Long Task**（长任务，阻塞主线程），会直接影响 **INP**（Interaction to Next Paint）与 **可交互时间**。
...
1. \==下载== script（Network）
2. **Parse + Compile**（解析与编译，大文件更明显，尤其移动设备）
...
| 框架 | 超大 **虚拟 DOM**（框架在内存中维护的 DOM 树表示）diff、错误导致的重复渲染 |
