...
---
长列表的本质矛盾：==DOM 数量 ∝ 数据行数==，而布局/绘制/ **diff**（虚拟 DOM 比对更新）成本随 DOM 涨。**图片**则是 **字节 × 解码 × 优先级**：**LCP**（Largest Contentful Paint，视口最大内容绘制）候选若被排到队尾，首屏体验直接崩。

***

### 一、虚拟滚动：固定行高（最易实现）

**虚拟滚动**（只渲染视口内 + 缓冲区的 DOM，用 padding/transform 模拟总高度）视口高 `H`，行高 `h`，缓冲 `b`，总行数 `N`：
