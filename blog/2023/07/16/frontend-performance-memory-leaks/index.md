...
---
泄漏 = ==应被回收的对象仍被 GC Root 可达==（**GC**，Garbage Collection，垃圾回收；**Root** 是仍被引用的起点，如全局、闭包、DOM）。SPA（Single Page Application，单页应用）里常见：**事件未解绑**、**定时器未清**、**全局 Map 无限涨**、**闭包抓住已卸下的 DOM**、**ObjectURL 未 revoke**。
