...
---
渲染管线（再简化仍有用）：==Style → Layout → Paint → Composite==（样式 → 布局/重排 → 绘制 → 合成层上 GPU 合成）。\
**Layout（重排）** 最贵：**几何信息**变了就要重新排版；**Paint** 次之；**Composite** 常由 **GPU** 完成，成本相对可控。
...
| transform、opacity（多数情况） | **Composite**（合成层，理想路径，常不触发布局） |
...
浏览器本可 **批量** 提交 DOM/CSS 变更，再在下一帧统一算布局；但 JS **读** `offsetWidth`、`scrollHeight`、`getBoundingClientRect()` 等时，必须先拿到 **最新布局**，于是 **强制提前 flush**——这就是 **强制同步布局**（Forced Synchronous Layout，又称 layout thrashing）。
