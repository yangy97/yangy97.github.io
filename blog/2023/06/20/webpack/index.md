...
* Webpack初始
  ...
  * 定义
    ...
    * \==webpack== 是一个用于现代 JavaScript 应用程序的**静态模块打包**（Static Module Bundler，把多文件依赖图打成浏览器可加载资源）工具
  ...
  * 特点
    ...
    * 构建（不支持的代码转换成支持的代码）
      ...
      * 源代码编译成浏览器能解析的生产代码（如：**ES6**=>**ES5**，**SCSS**=>**CSS**；常经 **Babel**、**Loader** 链完成）
    ...
    * 打包
      ...
      * 把多个文件合并成一个文件，减少 **HTTP 请求**的次数，提高生产环境的运行效率
...
* 加载器（Loader）
  ...
  * webpack 只能理解 JS 和 JSON 文件，**loader**（把非 JS 资源转成 webpack 可消费模块的转换器）让 webpack 能够去处理其他类型的文件，并将它们转换为有效模块，以供应用程序使用
