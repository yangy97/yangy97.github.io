...
---
## `vite build` 本质是 ==调 Rollup== 产出 **浏览器包**；**SSR**（Server-Side Rendering）、**库模式**、**Node 端打包** 时 **`ssr.noExternal` / `build.rollupOptions.external`**（外部化：不把该依赖打进 bundle）决定 **哪些依赖打进 bundle**，错配会导致 **双份 React** 或 **服务端读不到 CJS**。
