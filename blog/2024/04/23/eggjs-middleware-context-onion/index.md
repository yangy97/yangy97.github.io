...
---
## Egg 的请求管线建立在 ==Koa== 之上：**中间件** 是 `async (ctx, next) => {}`，`await next()` 把控制权交给 **下一层**，形成 **洋葱模型**（Onion Model，请求先进外层再进内层，响应沿相反顺序返回）。
...
* **`await next()`** 之前的代码在 **进入下游前** 执行；**之后** 的代码在 **下游返回后** 执行（若下游抛错，仍可能走到你 `try/finally` 里，取决于你是否捕获）。**`ctx`**（Context，单次请求的上下文，含 request/response/state）贯穿整条链。
