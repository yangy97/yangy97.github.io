...
---
## Egg 用 ==多文件 config== 表达 **默认 / 环境 / 本地**；**插件**（plugin，把数据库、Session 等能力注册进框架）把 **数据库、ORM、安全、Session** 等能力 **注册进应用**。
...
**环境**：`EGG_SERVER_ENV`（Egg 运行环境标识）决定加载哪档；与 **`NODE_ENV`**（Node 运行模式，如 production/development）不要混为一谈——部署脚本里 **打印当前 env** 能少一半扯皮。
