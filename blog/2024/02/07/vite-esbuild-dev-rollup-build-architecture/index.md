...
---
Vite 的「快」不是玄学：==开发态== 用 **esbuild**（Go 写的极速打包/转译器）做依赖预构建与 TS/JS 转译；**生产构建** 用 **Rollup**（以 ESM 为主的打包器）做 **tree-shaking**（摇掉未引用导出）、**拆 chunk**、**压缩** 等成熟管线。理解 **为什么 dev 不打包应用源码**、**预构建解决什么**、**build 与 dev 行为差异**，才能 **正确配置**、**少踩环境坑**。

***

### 一、开发时：原生 ESM + 按需编译

浏览器 ==直接请求== 源码 **ESM**（ECMAScript Module，浏览器原生 `import/export` 模块，路径经 Vite 解析），**改哪个文件编译哪个**，配合 **HTTP 缓存头**（304 Not Modified）与 **模块图**（依赖关系图），冷启动 **远小于** 打整包。
...
首次 `npm run dev` 时，Vite 扫描入口，对 **bare import**（不带 `./` 前缀的包名导入，如 `import lodash from 'lodash'`）：
