...
---
## ==Egg.js== 在 **Koa**（轻量 Node HTTP 框架）之上用 **约定目录 + Loader**（启动时扫描目录并挂载到 app/ctx 的加载器）把「路由、控制器、服务、中间件、插件」组装成可维护的后端。
...
| `app/middleware/foo.js` | 在 `config` 里 **按名启用** 后进入 **洋葱模型**（中间件链，先入后出） |
...
1. **Master / Agent**（多进程时：Master 管 Worker，Agent 跑定时/监控等单例任务）由 **cluster** 监听端口，**Worker** 里跑你的应用代码。
...
记四层：**Loader 挂载 → 中间件洋葱 → router → controller/service**。新人按目录找文件；老人改需求时先问 **该动 extend / 中间件 / service 哪一层**，避免在 controller 里堆业务。同仓前后端目录约定见[《从 0 到 1 实战：pnpm 同仓 + Vue3 Vite + Egg.js BFF》](/2024/03/03/eggjs-fullstack-vue-vite-bff-from-zero/)。
