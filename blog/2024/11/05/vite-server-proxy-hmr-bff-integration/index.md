...
---
## 本地开发时，浏览器访问 ==Vite 端口==（如 5173），接口走 **同源 `/api`** 再 **代理到后端**，可 **规避 CORS**。**HMR**（Hot Module Replacement，热模块替换）通过 **WebSocket** 推送 **模块更新**。
...
### 四、与 **BFF**（Backend For Frontend，为前端聚合接口的后端层）同仓
