...
---
## `http`/`https` 请求背后是一条条 ==TCP 连接==。**Agent**（HTTP 连接池管理器）管理 **连接池、复用、并发上限**；不配好会出现 **端口耗尽（TIME\_WAIT）**、**对单域名串行**、或 **泄漏的 socket**。

### 一、默认 Agent 在干什么

* \==`http.globalAgent`==（及 https 对应）维护 **对同一 host:port** 的 **空闲连接**，**Keep-Alive**（HTTP 持久连接，同 TCP 传多请求）下可 **复用**，减少握手。
