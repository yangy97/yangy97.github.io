...
---
## Vite 插件 ==兼容 **Rollup**（生产构建用的模块打包器）插件钩子==，并扩展 **`configureServer`**（仅开发期定制 dev 服务器）等钩子。理解 **`resolveId`（解析模块 ID）→ `load`（读取模块内容）→ `transform`（改写源码）** 三段，就能 **虚拟模块**、**替换环境常量**、**注入 HMR**（Hot Module Replacement，热模块替换）。
