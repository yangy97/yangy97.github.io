...
---
## 下面是一套 ==能直接照着搭== 的最小闭环：**根目录 pnpm workspace**，`apps/web` 跑 **Vite + Vue3**，`apps/server` 跑 **Egg 3**；浏览器只访问 **5173**，通过 **开发代理** 把 `/api` 转到 Egg **7001**，避免本地 **CORS**（Cross-Origin Resource Sharing，跨域资源共享限制）。
