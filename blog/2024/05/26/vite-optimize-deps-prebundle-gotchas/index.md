...
---
## 开发时 Vite 用 ==esbuild== 把 **node\_modules 里难直接给浏览器的依赖** 打成 **可缓存的 ESM 包**，写在 **`node_modules/.vite`**。**optimizeDeps**（依赖预构建）搞不清 **`include/exclude`**，会遇到 **`Failed to resolve import`**、**双份依赖**、**热更新慢**。

### 一、为什么要预构建

* \==CommonJS / 散落入口==：浏览器不能原生 `require`（CJS 模块系统）。
* **大量小文件**：直接请求 **deep path** 会 **请求风暴**。
* **统一 ESM**（ECMAScript Modules，浏览器可用的 `import`/`export` 模块格式）**形状**：后续 **转换成本** 更低。
