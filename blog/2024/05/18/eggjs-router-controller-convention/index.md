...
---
## Egg 推荐 ==`app/router.js` 集中声明==，由 **Controller**（控制器，承接 HTTP 参数并调用 service 返回 body）表达 HTTP 语义。
...
* **Query**（URL 查询字符串）→ `ctx.query`，注意 **数组与重复 key** 的解析规则。

#### 2）`router.resources`（**REST** CRUD 一条龙，Representational State Transfer，用 HTTP 动词操作资源）
