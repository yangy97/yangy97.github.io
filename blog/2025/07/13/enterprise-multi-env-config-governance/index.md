...
---
「同一套代码、多套运行环境」在团队变大后，痛点往往不是 ==有没有 dev/staging/prod==，而是：**配置从哪来、谁改过、和代码版本是否一致、有没有人在生产手改**。
...
|------|------|--------------|----------------|
| ==默认/非敏感== | 超时、功能开关默认值、日志级别 | 是（模板化） | **ConfigMap**（K8s 非敏感键值配置对象）、`values.yaml` |
| **环境差异** | DB 主机名、副本数、限流阈值 | 是（按环境文件） | `values-prod.yaml`、**Helm overlay**（Helm 按环境叠加的配置层） |
| **敏感** | 密码、证书私钥、第三方 API Key | **否**（仅存密管） | **Secret**（K8s 密钥对象）、**Vault**（集中密钥管理）、云 **KMS**（Key Management Service） |
...
3. **成熟期**：**GitOps**（Argo CD / Flux 等：Git 为集群期望状态唯一来源）——集群状态以 Git 为唯一事实来源，人工 `kubectl edit` 视为 **债务**。
...
* **流水线与制品晋升**：见[《企业级 CI/CD：流水线设计、门禁与发布治理》](/2025/04/09/enterprise-cicd-pipeline-deep/)。
* **入口与灰度**：见[《企业级应用接入与流量转发：网关、负载均衡与灰度》](/2025/06/11/enterprise-ingress-traffic-forwarding/)。
* **上线后指标**：见[《企业级流量监控与可观测性：指标、日志、追踪与 SLO》](/2025/08/14/enterprise-observability-traffic-monitoring/)。
