...
---
当 ==遗留系统完全不可改构建==、或 **合规要求强隔离**（不同安全域数据），**iframe**（浏览器原生文档隔离边界）仍是最稳的集成手段。主壳与子页用 **`postMessage`（跨窗口结构化消息 API）** 通信；登录与鉴权优先经 **BFF**（Backend For Frontend，为前端聚合接口、统一 Cookie/会话的后端层），避免在消息里传 token。代价是 **URL 同步、登录态、高度自适应、弹层遮罩** 要自己做。下面给 **可拷贝的协议与代码骨架**。
...
**`postMessage`** 必须校验 **`event.origin`**，只接受白名单域；消息体用 **`type` + `version`** 做向前兼容。
