...
* \==**定位**（应用级编排，与 ice.js 工程一体）==：与 **ice.js**、**微模块（icestark module，页面内区块远程加载）** 同一套叙事，提供 **子应用注册、缓存、加载策略**，文档入口见 [iceworks / micro-frontends](https://micro-frontends.ice.work/)。
...
* **定位**：在 **Module Federation（MF，模块联邦：exposes/remotes/shared）** 之上提供 **CLI、共享依赖分析、多项目模板**（实现层多为 **Webpack 5** 系），官网 [empjs.dev](https://empjs.dev/)。**业务子工程** 若可自选，**联邦消费侧/供应侧** 也可对照 **Vite 联邦** 的 `shared` 思路，不必绑死一种打包器。
...
* **定位**：在 **monorepo**（单 Git 仓库管理多包/多应用）里维护 host/remote，**依赖图、缓存、受影响分析** 由 Nx 负责，[Nx Module Federation](https://nx.dev/concepts/module-federation)。
