...
---
**Module Federation（MF，模块联邦）** 最初是 **Webpack 5** 特性：**运行时** 从 **remote 容器** 加载 **`exposes`（对外暴露的模块路径）** 的模块，**`shared`（共享依赖声明，常配 singleton 单例）** 避免 React/Vue 双份。**现在落地新项目更常用 Vite 联邦插件**（`@originjs/vite-plugin-federation` 等），**字段语义与下述 Webpack 一致**，差别只在写在 **`vite.config.ts`** 还是 **`webpack.config.js`**。与 **qiankun 应用级** 不同，MF 更偏 **模块/组件级集成**。
...
| 角色 | 职责 |
|------|------|
| ==**Remote**（供应方，构建时 `exposes`）== | `exposes` 对外暴露 `./Button`、`./Page` |
| **Host**（消费方，构建时 `remotes`） | `remotes` 声明远程入口，**`import('remoteName/exposed')`** |

**运行时**：Host 加载 **`remoteEntry.js`（Remote 容器入口清单）**，再 **异步拉取 chunk**。
