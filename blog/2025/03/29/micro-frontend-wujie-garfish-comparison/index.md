...
---
国内常见 **微前端**（浏览器内组合多独立 SPA）方案除 **qiankun**（single-spa + HTML Entry + JS 沙箱）外，还有 **wujie（无界）**、**Garfish**、**micro-app** 等；底层差异集中在 **沙箱实现、样式隔离、对子应用（Vite 为主、Webpack 老栈）的适配、预加载策略**。下面做 **能力向对比** 与 **选型建议**（以公开文档为准，版本迭代快，接入前请对照官方）。
...
| 维度 | 关注点 |
|------|--------|
| ==**隔离**（JS/样式/全局对象是否互不影响）== | JS / 样式 / 路由是否 **可逆**、**第三方弹层** 是否翻车 |
| **性能** | 首屏、切换子应用、**多实例内存** |
...
* **成熟**、案例多；**single-spa**（`activeWhen` + `registerApplication`）模型清晰。
* 子应用需 **`bootstrap/mount/unmount` 生命周期** 与 **publicPath** 等适配；**Vite** 需 **`vite-plugin-qiankun`** 等社区插件。

***

### 三、wujie（无界）：WebComponent + iframe 思路变种

公开资料强调：**Web Components**（浏览器自定义元素 + Shadow DOM 标准）容器 + **子应用运行在 iframe**（或类 iframe 环境）带来的 **强隔离**，主应用通过 **代理** 同步路由与 **注入**。\
