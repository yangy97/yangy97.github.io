...
---
## Playwright 不只是「能点页面的脚本」：它把 ==Browser → Context → Page== 分层、内置 **自动等待**（断言前自动轮询至可操作）、**Trace**（执行录像/网络/快照，用于排障）/视频 与 **多浏览器** 抽象。
...
| **BrowserContext** | **隔离** 的会话：Cookie、Storage、权限（比开新 Browser 轻） | 多用户并行、无痕、不同 `baseURL` |
...
Playwright 推荐 **`getByRole`、`getByLabel`、`getByText`**（与 **Testing Library**（React Testing Library 系，按角色/标签查询 DOM）可访问性优先理念一致），避免 `#app > div:nth-child(3)` 这类 **布局一变就碎** 的选择器。
