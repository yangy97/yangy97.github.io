...
* \==**host**（消费方 Host，运行时拉远程模块）==：运行时拉 **`remoteEntry.js`（Remote 容器入口，列出 exposes 清单）**。
* **remote**（供应方 Remote）：**`exposes`** 暴露页面或组件路径。
* **约定**：**React / Vue 大版本** 在 **`shared`（联邦共享依赖，常配 singleton 单例）** 与 **子项目 `peerDependencies`** 中 **锁死**。
