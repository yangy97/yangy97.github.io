...
* \==**主应用（基座）**（Shell：菜单、登录、子应用注册与挂载容器）==：负责 **菜单、登录、子应用注册、容器 DOM**；**顶层路由** 必须能落到同一 HTML（**history fallback**，刷新子路径仍返回壳 HTML）。
* **子应用**：可 **独立开发、独立部署**；必须导出 **`bootstrap` / `mount` / `unmount`**（**生命周期**：qiankun 按序加载、挂载、卸载子应用 SPA）。
* **建议约定**：子应用 **URL 前缀** 如 `/app-order`，与 **`router` base**、**`activeRule`（路由匹配规则，命中时激活子应用）**、**`base`（Vite publicPath，静态资源前缀）** 三处写同一个字符串。

**端口示例**：主应用 `8080`，子应用 A `8081`、B `8082`（`registerMicroApps` 里多一条即可）。

***

### 二、主应用：安装与启动 **qiankun**（基于 single-spa 的微前端框架，HTML Entry + JS 沙箱）
