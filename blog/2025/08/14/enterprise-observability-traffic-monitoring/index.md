...
---
「企业级」运维通常要求：==知道系统是否在 **SLO**（Service Level Objective，对外承诺的服务目标）内、出问题能分钟级定位、事后能复盘==。可观测性三板斧：**Metrics**（指标）、**Logs**（日志）、**Traces**（分布式追踪）；流量侧则强调 **黄金信号、RED/USE 方法、告警降噪**。
...
| 信号 | 含义 | 典型指标 |
|------|------|----------|
| **Latency** | 延迟 | **P50/P95/P99**（第 50/95/99 分位延迟，比平均值更能反映尾部）、按路由拆分 |
| **Traffic** | 流量 | QPS、并发连接 |
...
* **RED**（面向请求的服务）：**R**equest rate（请求速率）、**E**rror rate（错误率）、**D**uration（耗时）。适合 API、网关。
* **USE**（面向资源）：**U**tilization（利用率）、**S**aturation（饱和度/排队）、**E**rrors（错误）。适合机器、磁盘、网络。
...
| 组件 | 角色 |
|------|------|
| **Prometheus** | **pull**（拉取，定时从目标抓取）时序指标、**PromQL**（Prometheus 查询语言）查询 |
| **Grafana** | 仪表盘与告警展示 |
...
见[《企业级 CI/CD：流水线设计、门禁与发布治理》](/2025/04/09/enterprise-cicd-pipeline-deep/)。
