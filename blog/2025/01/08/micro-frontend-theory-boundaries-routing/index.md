...
---
**微前端**（在浏览器里组合多个可独立交付、独立发版的前端应用）不是「把一个大 SPA（Single Page Application，单页应用）拆成多个仓库」这么简单，还要同时处理 **路由、样式、JS 全局污染、版本共存、发布节奏**。**主应用（基座）**（Shell：菜单、登录与子应用挂载容器）负责壳层路由与挂载点；**子应用** 可异构技术栈、独立发版。下面从 **何时值得做、拆分维度、路由策略、应用间通信** 写理论层，并给 **决策表**。
...
| **增量迁移** | **绞杀者模式**（Strangler Fig Pattern，逐步用新模块替换旧系统边角的迁移策略）：一块块替换遗留页面 |
...
**basename**（路由公共前缀）在子路径方案里必须与网关、子应用 `router.base` 一致，否则刷新 404。
...
**activeRule**（路由前缀或函数，命中时挂载子应用）/ **basename** 决定子应用何时挂载；**history 模式** 下壳层必须配置 **fallback** 把未知路径交回主应用 HTML。
...
| **BFF**（Backend For Frontend，为前端聚合接口、统一鉴权的后端层） | 数据 **以服务端为真**，前端只展示 |
...
* **Shadow DOM**（影子 DOM，样式与节点树与主文档隔离）：**强隔离**，但 **第三方组件**（如弹层挂 body）常 **穿透** 出问题。
...
微前端理论层 = **拆分是否值得 + 路由归谁 + 通信与全局单例策略**；落地时还要补上 **网关路径、publicPath、团队契约**。**主流方案索引** 见[《微前端主流方案版图与选型（2026）》](/2025/04/24/micro-frontend-mainstream-landscape-and-choice-2026/)；实操系列见 **qiankun**、**single-spa**、**Module Federation**、**iframe + postMessage** 各篇。
