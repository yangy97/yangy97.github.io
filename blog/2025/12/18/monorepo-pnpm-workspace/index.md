...
---
## ==Monorepo== 指多个包/应用放在 **同一个 Git 仓库** 里统一版本管理与协作。**pnpm** 通过 **workspace**（工作区，本地包软链接）把本地包链接起来，安装快、磁盘省。
...
代价：**仓库变大**、CI 要会做 **变更检测**（只测改动的包，常配合 **turbo/nx** 等任务编排工具），否则全量跑很慢。
...
于是 `apps/web`、`packages/ui` 等互为 **workspace 包**，本地依赖可写 `"@scope/ui": "workspace:*"`（`workspace:` 协议表示链接 monorepo 内包）。
