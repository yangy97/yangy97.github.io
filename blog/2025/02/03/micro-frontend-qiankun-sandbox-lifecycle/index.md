...
---
[**qiankun**](https://qiankun.umijs.org/)（蚂蚁微前端框架）基于 **single-spa**（多应用路由编排库），在工程上补齐 **HTML Entry**（通过子应用 `index.html` 加载脚本）、**JS 沙箱**（隔离全局变量）、**样式隔离**、**预加载**。下面写 **注册参数、子应用导出协议、沙箱类型与限制、全局状态**，并带 **配置示例**。
...
2. **再建主应用**（Vue3/React 均可），安装 `qiankun`，在布局里留出 `#subapp-viewport`（子应用挂载点 DOM）。
3. **子应用改 `router` base** 与主应用 **`activeRule`（路由前缀或函数，命中时挂载子应用）** 一致，例如 basename `/vue`。
4. **子应用入口导出** `bootstrap/mount/unmount`（见下文；**Vite 用** `vite-plugin-qiankun` 的 `renderWithQiankun`），`npm run dev` 两端口联调。

**`entry`**（子应用入口地址）：可以是 **远程部署的 index.html**，实现 **独立部署**；本地联调写 `//localhost:8081` 即可（`//` 表示跟随当前页面协议）。

***

### 二、主应用注册（完整示意）

`registerMicroApps` 为每个子应用绑定 **`name`、`entry`、`container`、`activeRule`**；`start` 开启路由监听与沙箱。
