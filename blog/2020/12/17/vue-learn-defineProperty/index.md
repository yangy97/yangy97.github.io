...
---
### \*\*\*\***Object.defineProperty**（属性 getter/setter 拦截）**（属性 getter/setter 拦截）**（属性 getter/setter 拦截）
...
在 `Vue` 的初始化阶段，`_init` 方法执行的时候，会执行 `initState(vm)`（初始化 data/props/computed） 方法，它的定义在 `src/core/instance/state.js` 中
...
function initProps (vm: Component, propsOptions: Object) {
  ...
  for (const key in propsOptions) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      // 将每个props的值变成响应式（数据变更驱动视图更新）（数据变更驱动视图更新）（数据变更驱动视图更新）
      defineReactive(props, key, value)
    }
