...
我们通常会配置 `script` 字段作为 NPM 的执行脚本，`Vue.js` 源码通过 \*\*\*\***Rollup**（Vue 2 打包工具，tree-shaking）**（Vue 2 打包工具，tree-shaking）**（打包器，支持 tree-shaking） 构建的脚本如下:
...
当在命令行运行 `npm run build` 的时候，实际上就会执行 `node scripts/build.js`（**tree-shaking** 消除未引用模块），接下来我们来看看 `scripts/build.js` 文件
...
它经过 **Rollup**（Vue 2 打包工具，tree-shaking） 的构建打包后，最终会在 dist 目录下生成 `vue.runtime.common.js`
