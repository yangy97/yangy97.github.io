...
> 如果容器可以滚动，在 **IE**（Internet Explorer）和 **FireFox** 下 `padding-bottom` 会被忽略，`chrome` 不会

> chrome 是子元素超过 **content box**（内容盒，不含 padding）触发滚动条显示；在 IE 和 FireFox 是 **padding box**（含 padding 的盒模型）会触发滚动条

> `建议: 页面底部留白不建议用padding`

### margin 合并规则

> **margin 折叠**（相邻块级元素垂直 margin 合并为较大值）：正正取最大，正负值相加，负负取最负值
