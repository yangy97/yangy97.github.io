...
---
## 目标：不用脚手架，用 ==webpack 5== 搭一个 **能改即热更新** 的前端入口。搞清 **entry**（入口，构建图起点）、**output**（输出路径与文件名）、**mode**（development/production，影响压缩与行为）、**devServer**（开发服务器，内存 bundle + HMR），后面读任何配置都有锚点。
...
module.exports = {
  ...
  output: {
    ...
    filename: 'bundle.[contenthash:8].js', // contenthash：内容变才变 hash，利于长期缓存
    clean: true,
  ...
  devServer: {
    ...
    port: 8080,
    hot: true,
    historyApiFallback: true, // History 路由下未知路径回退 index.html，避免刷新 404
  },
  plugins: [
