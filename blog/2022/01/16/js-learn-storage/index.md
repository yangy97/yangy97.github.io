...
---
浏览器存储是前端面试与 **鉴权方案** 的常客。本篇在红宝书笔记基础上，补 \*\*\*\*****Cookie**（可设 HttpOnly/过期）**（可设 HttpOnly/过期）**（可设 HttpOnly/过期） 属性链、Storage 边界、安全默认值**，并与 [Token 存储](/2023/05/22/frontend-auth-token-storage-refresh-xss/) 文衔接。
...
```javascript
******localStorage**（持久键值，同源）**（持久键值，同源）**（持久键值，同源）.setItem('key', JSON.stringify(value));
...
| `removeItem(name)` | 删除 |
| `key(index)` | 按序取下标名 |
| `clear()` | 清空（Firefox 对 \*\*\*\***sessionStorage**（标签页级键值）**（标签页级键值）**（标签页级键值） 行为曾不一致，以现行为准） |
