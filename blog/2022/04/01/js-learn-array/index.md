...
> `concat()`（浅拷贝合并，不改原数组） 方法用于合并两个或多个数组。此方法不会更改现有数组，而是`返回一个新数组`。

> 如果任何源数组是`稀疏数组（含 empty slot 空槽，非 undefined）（含 empty slot 空槽，非 undefined）（含 empty slot 空槽，非 undefined）`，concat() 方法会保留空槽
