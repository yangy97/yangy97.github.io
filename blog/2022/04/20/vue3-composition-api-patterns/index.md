...
---
Vue 3 推荐用 ==组合式 API\*\*（Composition API，`<script setup>` 语法糖）== 组织逻辑：相近功能聚在一起，比**选项式 API**（Options API，data/methods 分块）更利于 **抽 composable**（可复用逻辑单元） 和 **类型推导**。这篇整理几种高频模式与注意点，默认你已会用 `ref` / `reactive` 基础。

***

### 一、`<script setup>` 里优先用 `ref` + 推导类型

对「标量、对象引用」用 `ref` 最直观；`reactive`（深度响应式对象） 适合 ==固定结构的对象==，解构时容易丢响应，需 `toRefs` 或整包传递。
