...
---
## Loader 把 ==任意资源== 转成 Webpack 能继续解析的 **JS 模块字符串**；**从右到左** 的执行顺序常被背错，而 **Pitch** 阶段则是 **从左到右** 的另一条链。**Tree shaking**（基于 ESM 静态分析摇掉未用导出）依赖 **ESM 静态结构** 与 `package.json` 的 **`sideEffects`**（声明哪些文件有副作用、不可删）声明。
