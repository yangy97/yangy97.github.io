...
---
## ==**resolve**（模块寻址配置，决定 `import` 落在哪个文件）== 决定 `import './foo'` 最终落到哪个文件；配错会出现 **打包进两份 React**、**tree-shaking**（Tree Shaking，摇掉未用导出以减体积）失效、**条件导出**（package.json `exports` 按环境选入口）走错入口。
...
`package.json` 里常有 **`module`、`main`、`browser`**。Webpack 默认顺序大致为 **`['browser', 'module', 'main']`**（以版本文档为准）。**mainFields**（解析 package.json 时优先读哪个入口字段）决定走哪条路径。
