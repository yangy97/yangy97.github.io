...
---
## 生产构建关注 ==体积、缓存、可观测性==；**source map**（源码到产物的行号映射）决定 **线上报错能否映射回源码**，也影响 **泄露源码** 风险。
...
| **hidden-source-map** | 较快 | 完整映射 | **只上传 Sentry** 等，浏览器 **不暴露 map 文件** |
...
`production` 默认 **TerserPlugin**（JS 压缩/混淆）压缩；若 **堆栈行号对不上**，检查 **devtool 是否关闭** 或 **是否用了 cheap** 导致 **列信息缺失**。
